...
if [[ -f "src/index.html" ]]; then
    ...
    if grep -q "برنامج تنظيم الأضاحي الإسلامية" src/index.html; then
    ...
    if grep -q "firebase" src/index.html; then
...
if [[ -f "src/css/style.css" ]]; then
    ...
    if grep -q "direction: rtl" src/css/style.css; then
...
start_server() {
    ...
    if command -v python3 &> /dev/null; then
        ...
        echo "الخادم يعمل على: http://localhost:8000/src/index.html"
        echo "Server running at: http://localhost:8000/src/index.html"
        ...
        print_info "يرجى فتح src/index.html مباشرة في المتصفح"
        print_info "Please open src/index.html directly in your browser"
    fi
}

# Function to validate Firebase config
check_firebase_config() {
    if grep -q "YOUR_API_KEY" src/js/firebase-config.js; then
        print_warning "إعدادات Firebase لم يتم تحديثها - Firebase config not updated"
        print_info "يرجى تحديث src/js/firebase-config.js بإعدادات مشروعك"
        print_info "Please update src/js/firebase-config.js with your project settings"
    else
